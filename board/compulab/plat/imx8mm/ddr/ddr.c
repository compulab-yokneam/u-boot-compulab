...
static int find_timing_block(unsigned long id)
{
	...
	if(!strncmp(timing_block.magic, LPDDR_SINGLE_MAGIC, sizeof(LPDDR_SINGLE_MAGIC))) {
		printf("Single timing block 0x%x incorporated ", timing_block.id);
	...
	// No incorporated timing present -- search on a storage
