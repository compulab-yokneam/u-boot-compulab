...
static int imx8m_pm_domain_on(struct power_domain *power_domain)
{
	...
	if (pd->regulator) {
		...
		if (ret) {
			printf("failed to power up the regulator%d\n", ret);
			return ret;
		}
	}
...
static int _imx8m_pm_domain_off(struct udevice *dev)
{
	...
	if (pd->regulator) {
		...
		if (ret) {
			printf("failed to power off the regulator%d\n", ret);
			return ret;
		}
	}
...
static int imx8m_pm_domain_ofdata_to_platdata(struct udevice *dev)
{
	...
	if (!pd->name) {
		printf("failed to get the domain name\n");
		return -EINVAL;
	}

	ret = ofnode_read_u32(np, "domain-index", &pd->domain_index);
	if (ret) {
		printf("failed to get the domain index\n");
		return -EINVAL;
	}
	...
	if (IS_ENABLED(CONFIG_DM_REGULATOR)) {
		...
		if (ret) {
			if (ret != -ENOENT) {
				printf("%s: cannot get power regulator: ret=%d\n",
				      __func__, ret);
				return ret;
			}
		}
	...
	if (ret) {
		printf("failed to get domain's clocks\n");
		return ret;
	}
	...
	if (!ofnode_parse_phandle_with_args(np, "parent-domains",
		...
		ret = power_domain_get_by_args(&pd->parent_pd, &parent);
	        if (ret) {
	                printf("power_domain_request_by_args failed: %d\n", ret);
	                return ret;
	        }
	}
	return 0;
